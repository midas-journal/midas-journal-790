...
static void vtkGetBSplineWeights(double F[4], double G[4], double f)
{
  ...
  double *ff[3], double *gg[3], int ext[3], int borderMode)
{
  ...
  for (int i = 0; i < 3; i++)
      ...
      switch (borderMode)
        ...
        case VTK_BSPLINE_ZERO:
          ...
          if (gridId1[i] == 0)
            ...
            if (gg[i]) { gg[i][0] = 0.0; gg[i][1] = 0.0; }
            }
          else if (gridId3[i] == 0)
            {
            ff[i][0] = 0.0; ff[i][1] = 0.0; ff[i][2] = 0.0;
            if (gg[i]) { gg[i][0] = 0.0; gg[i][1] = 0.0; gg[i][2] = 0.0; }
            }
          else if (gridId3[i] < 0)
            {
            pointIsInvalid = 1;
          ...
          if (gridId2[i] == ext[i])
            ...
            if (gg[i]) { gg[i][2] = 0.0; gg[i][3] = 0.0; }
            }
          else if (gridId0[i] == ext[i])
            {
            ff[i][1] = 0.0; ff[i][2] = 0.0; ff[i][3] = 0.0;
            if (gg[i]) { gg[i][1] = 0.0; gg[i][2] = 0.0; gg[i][3] = 0.0; }
            }
          else if (gridId0[i] > ext[i])
            {
            pointIsInvalid = 1;
    ...
    if (gridId0[i] < 0) { gridId0[i] = 0; }
    if (gridId0[i] > ext[i]) { gridId0[i] = ext[i]; }
    if (gridId1[i] < 0) { gridId1[i] = 0; }
    if (gridId1[i] > ext[i]) { gridId1[i] = ext[i]; }
    if (gridId2[i] < 0) { gridId2[i] = 0; }
    if (gridId2[i] > ext[i]) { gridId2[i] = ext[i]; }
    if (gridId3[i] < 0) { gridId3[i] = 0; }
    if (gridId3[i] > ext[i]) { gridId3[i] = ext[i]; }
    }
...
template <class T>
void vtkBSplineCubicInterpolation(
...
template<class T>
void vtkCalculateBSpline<T>::Cubic(
  const double point[3], double displacement[3], double derivatives[3][3],
  void *gridPtrVoid, int gridExt[6], vtkIdType gridInc[3], int borderMode)
{
  ...
  for (int i = 0; i < 3; i++)
    ...
    if (ext[i] != 0)
      ...
      vtkGetBSplineWeights(ff[i], gg[i], f);
      }
    }
  ...
  if ((gridId0[0] | (ext[0] - gridId3[0]) |
    ...
    {
    pointIsInvalid = vtkBSplineBorder(gridId0, gridId1, gridId2, gridId3,
                                      ff, gg, ext, borderMode);
    }
...
void vtkBSplineTransform::InternalUpdate()
{
  ...
  switch (grid->GetScalarType())
    {
    case VTK_FLOAT:
      this->CalculateSpline = &(vtkCalculateBSpline<float>::Cubic);
      break;
    case VTK_DOUBLE:
      this->CalculateSpline = &(vtkCalculateBSpline<double>::Cubic);
      break;
    default:
      this->CalculateSpline = 0;
